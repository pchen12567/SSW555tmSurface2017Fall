"""
Created on Mar 5 17:40:22 2018
@author: Pan Chen
This program provides a function that implement US21 in Sprint 1.
Refactor program from 'US21_BAD.py':
1) Create a new function sub_correct_gender() to simplify the duplicated code.
2) Rename the function correct_gender(info) to us21_correct_gender(info)
...
def us21_correct_gender(info):
    ...
    for i in fams:
        ...
        rt += sub_correct_gender(fam, indis, "HUSB", 'M')
        # id_husb = fams[i]['HUSB']
        ...
        rt += sub_correct_gender(fam, indis, "WIFE", 'F')
        # id_wife = fams[i]['WIFE']
...
def sub_correct_gender(fam, indis, character, gender):
    rt = []
    if indis[fam[character]]['SEX'] != gender:
        rt.append(
            {'error': 'ERROR', 'scope': 'INDIVIDUAL', 'user_story': 'US' + str(user_story),
             'line_number': indis[fam[character]]['line'],
             'id': indis[fam[character]]['INDI'], 'description': character + ' gender should be ' + gender})
    return rt
...
class CorrectGenderTest(unittest.TestCase):
    ...
    def test_us21_correct_gender001(self):
        self.assertEqual(us21_correct_gender(self.info1), [])

    def test_us21_correct_gender002(self):
        self.assertEqual(us21_correct_gender(self.info2), [{'description': 'HUSB gender should be M',
                                                            'error': 'ERROR',
                                                            'id': 'I01',
                                                            'line_number': 5,
                                                            'scope': 'INDIVIDUAL',
